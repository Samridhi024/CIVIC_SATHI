...
/* getTrafficProfile.js */

// 1. The Road Data (includes your new Patia road)
const roadData = {
  "type": "FeatureCollection",
  "features": [
    {
  ...
  for (const feature of roadData.features) {
    ...
    for (let i = 0; i < coordinates.length - 1; i++) {
      ...
      if (distDegrees < 0.0025) {
        ...
        if (roadType === 'trunk' || roadName.includes("VIP")) {
          urgency = "CRITICAL";
          desc = "MAIN HIGHWAY / VIP ROUTE DETECTED.";
          color = "danger"; // Red
        }
        ...
        return {
          level: urgency,
          label: `${roadName.toUpperCase()} DETECTED`,
          description: desc,
          color: color,
          roadName: roadName
        };
