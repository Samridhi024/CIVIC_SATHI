...
const IssueDetailsPage = () => {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                if (issueData.location) {
                    ...
                    if (isRoadIssue) {
                        setTrafficData(getTrafficProfile(issueData.location));
                    }
    ...
    const getSuggestedPriority = () => {
        ...
        if (aiInsights.some(insight => insight.type === 'Disaster')) {
    ...
    const handleUpdate = async () => {
        try {
            ...
            if (status === "Resolved") {
                ...
                alert('Issue Resolved & Email Sent! ✅');
            } else {
        ...
        <div className="issue-details-page p-4">
            ...
            <div className="d-flex justify-content-between align-items-start mb-3">
                <div>
                    ...
                    <Badge bg="secondary" className="me-2">{issue.type}</Badge>
                    <Badge bg={status === 'Resolved' ? 'success' : 'warning'} text="dark">{status}</Badge>
                </div>
                <div>
                    <Badge bg="success" className="me-2 p-2"><FaCheckCircle/> Verified: {verificationScore}</Badge>
                    <Badge bg="danger" className="p-2"><FaFlag/> Flags: {flagCount}</Badge>
                </div>
            </div>
            ...
            <Row className="g-4">
                <Col md={8}>
                    {/* Traffic Oracle */}
                    {trafficData && (
                        <Alert variant={trafficData.color} className="shadow-sm">
                            <FaRoad className="me-2"/> <strong>{trafficData.label}:</strong> {trafficData.description}
                        </Alert>
                    )}
                    ...
                    {aiInsights.length > 0 && (
                        <Card className="mb-4 shadow-sm border-0 bg-light">
                            <Card.Body>
                                <h6 className="text-primary fw-bold"><FaRobot className="me-2"/>AI Context Analysis</h6>
                                {aiInsights.map((insight, idx) => (
                                    <Alert key={idx} variant={insight.color} className="py-2 mb-2 small">
                                        <FaExclamationTriangle className="me-2"/> {insight.message}
                                    </Alert>
                                ))}
                            </Card.Body>
                        </Card>
                    ...
                    <Card className="shadow-sm mb-4">
                        <Card.Body>
                            ...
                            <hr/>
                            <p className="small text-muted">Location: {issue.location}</p>
                            <p className="small text-muted">Reported: {issue.createdAtFormatted}</p>
                        </Card.Body>
                    </Card>
                    ...
                    {issue.image && (
                        <Card className="shadow-sm">
                            <Card.Body>
                                <h5>Evidence</h5>
                                <img src={issue.image} alt="Evidence" className="img-fluid rounded" />
                            </Card.Body>
                        </Card>
                    )}
                </Col>
                        ...
                        <Card.Body>
                            {suggestion && (
                                <Alert variant="info" className="py-2 small">
                                    <FaLightbulb className="me-1"/> AI Suggests <strong>{suggestion.level}</strong> Priority ({suggestion.reason})
                                </Alert>
                            )}
...
//                     // 2. Context Logic
//                     const insights = analyzeIssueContext(issueData);
//                     setAiInsights(insights);
//                 }
...
//         // 1. Critical Traffic (VIP Routes)
//         if (trafficData && (trafficData.level === 'CRITICAL' || trafficData.level === 'HIGH')) {
//             return { level: 'Critical', reason: 'Heavy Traffic / VIP Route detected' };
//         }
           
//         // 2. Critical Context (Monsoon OR Child Safety)
//         const criticalInsight = aiInsights.find(insight => insight.color === 'danger');
//         if (criticalInsight) {
//              return { level: 'Critical', reason: criticalInsight.title };
//         }

//         // 3. Generic Safety Fallback
//         if (['Pothole', 'Accident', 'Streetlight'].includes(issue.type)) {
//             return { level: 'High', reason: 'General Safety Hazard' };
//         }
...
//             if (status === "Resolved") {
//                 // FIXED: Pass image/audio links to EmailJS
//                 // Note: Make sure your EmailJS template uses {{issue_image}} and {{issue_audio}}
//                 const emailParams = {
//                     user_name: "Citizen",
//                     user_email: issue.userEmail,
//                     issue_desc: issue.description,
...
//                     resolution_date: new Date().toLocaleDateString(),
//                     issue_image: issue.image || "No image provided",
//                     issue_audio: issue.audio || "No audio provided"
//                 };
...
//                     alert('Updated, but Email Failed (Check API Keys).');
//                 }
//             } else {
...
//     // FIXED: Build Gmail body string to include links
//     let gmailLink = '';
//     if (departmentEmail) {
//         const subject = `Regarding Issue ID: ${id} - ${issue.type}`;
        
//         let body = `Review Request:\nID: ${id}\nType: ${issue.type}\nLocation: ${issue.location}\n\nDesc:\n${issue.description}`;
...
//                     // 2. Context Logic (FIXED: Now Synchronous)
//                     const insights = analyzeIssueContext(issueData);
//                     setAiInsights(insights);
//                 }
...
//     // 👇 UPDATED LOGIC: Prioritize AI Critical Alerts over Generic Rules
//     const getSuggestedPriority = () => {
//         if (!issue) return null;
...
//         // 2. Critical Context (Monsoon OR Child Safety)
//         // Checks if any insight is RED (danger)
//         const criticalInsight = aiInsights.find(insight => insight.color === 'danger');
//         if (criticalInsight) {
//              return { level: 'Critical', reason: criticalInsight.title };
//         }
