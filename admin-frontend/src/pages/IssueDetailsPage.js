...
const IssueDetailsPage = () => {
    ...
    // FIXED: Professional Email Structure (No Emojis)
    let gmailLink = '';
    if (departmentEmail) {
        // 1. Formal Subject Line
        const subject = `[TICKET #${id}] ACTION REQUIRED: ${issue.type} Report`;
        
        // 2. Structured Body with Standard Text Separators
        let body = `To the Concerned Department,

This is an automated notification regarding a civic issue reported within your jurisdiction. Please review the details below and take necessary action.
...
Date Reported  : ${issue.createdAtFormatted}
...
Map Link       : http://googleusercontent.com/maps.google.com/maps?q=${encodeURIComponent(issue.location)}

------------------------------------------------------------
CITIZEN REPORT DESCRIPTION
------------------------------------------------------------
"${issue.description || 'No description provided by the citizen.'}"
...
`;
        ...
        // 3. Add Media Links (Text Only)
        if (issue.image) {
            body += `Photo Evidence Link:\n${issue.image}\n\n`;
        } else {
            body += `Photo Evidence: Not available\n\n`;
        }

        if (issue.audio) {
            body += `Audio Clip Link:\n${issue.audio}\n\n`;
        }

        // 4. Administrative Footer
        body += `------------------------------------------------------------
Civic Sathi Command Center
Automated Dispatch System
Please update the ticket status in the admin dashboard upon resolution.`;
